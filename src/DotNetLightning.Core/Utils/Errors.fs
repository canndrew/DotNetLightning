...
module OnionError =
    ...
    type FailureCode = {
        ...
        member this.GetOnionErrorDescription() =
            ...
            | (PERMANENT_CHANNEL_FAILURE) -> ("Node indicated the outgoing channel is unable to handle the HTLC permanently", "permanent_channel_failure")
            ...
            | (UNKNOWN_NEXT_PEER) -> ("Node indicated the outbound channel is not found for the specified short_channel_id in the onion packet", "unknown_next_peer")
            | (AMOUNT_BELOW_MINIMUM) -> ("Node indicated the HTLC amount was below the required minimum for the outbound channel", "amount_below_minimum")
