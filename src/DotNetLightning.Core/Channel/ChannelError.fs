...
module internal AcceptChannelMsgValidation =
    ...
    let checkConfigPermits (config: ChannelHandshakeLimits) (msg: AcceptChannelMsg) =
        ...
        let check7 = check (msg.MinimumDepth.Blocks) (>) (config.MaxMinimumDepth.Blocks |> uint32) "We consider the minimum depth (%A) to be unreasonably large. Our max minimum depth is (%A)"
