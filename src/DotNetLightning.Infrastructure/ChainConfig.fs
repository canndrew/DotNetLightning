...
type ChainConfig() =
    ...
    member val ToRemoteDelayBlocks: BlockHeightOffset16 = BlockHeightOffset16.FromBlocks 720us with get, set
    member val MaxToLocalDelayBlocks: BlockHeightOffset16 = BlockHeightOffset16.FromBlocks 2016us with get, set
    
    /// The number of confirmation when we consider the funding tx is locked.
    member val MinimumDepth = BlockHeightOffset32.FromBlocks 3u with get, set
    /// We do not accept channel if other node wants to wait more than this (in block number) before funding tx is locked.
    member val MaxMinimumDepth  = BlockHeightOffset32.FromBlocks 144u with get, set
