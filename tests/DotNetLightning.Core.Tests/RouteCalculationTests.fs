...
let tests = testList "Route Calculation" [
    let calculateRouteSimple routeParams =
        ...
        let route =
            Routing.findRoute (g) (a) (e) DEFAULT_AMOUNT_MSAT 1 (Set.empty) (Set.empty) (Set.empty) routeParams (BlockHeight.FromBlocks 400000u)
            |> Result.deref
    ...
    testCase "Calculate the shortest path (correct fees)" <| fun _ ->
        ...
        let route =
            Routing.findRoute graph a d amount 1 (Set.empty) (Set.empty) (Set.empty) DEFAULT_ROUTE_PARAMS (BlockHeight.FromBlocks 400000u)
            |> Result.deref
        ...
        let route1 =
            Routing.findRoute(graph1) a d amount 1  (Set.empty) (Set.empty) (Set.empty) DEFAULT_ROUTE_PARAMS (BlockHeight.FromBlocks 400000u)
            |> Result.deref
    ...
    testCase "calculate route considering the direct channel pays no fees" <| fun _ ->
        ...
        let g = DirectedLNGraph.Create().AddEdges(updates)
        let route =
            Routing.findRoute(g) a e DEFAULT_AMOUNT_MSAT 1 (Set.empty) (Set.empty) (Set.empty) DEFAULT_ROUTE_PARAMS (BlockHeight.FromBlocks 400000u)
            |> Result.deref
    ...
    testCase "Calculate simple route (add and remove edges)" <| fun _ ->
        ...
        let route2 =
            Routing.findRoute(graphWithRemovedEdge) a e DEFAULT_AMOUNT_MSAT 1 (Set.empty) (Set.empty)(Set.empty)DEFAULT_ROUTE_PARAMS (BlockHeight.FromBlocks 400000u)
    ...
    testCase "calculate the shortest path (select direct channel)" <| fun _ ->
        ...
        let graph = DirectedLNGraph.Create().AddEdges(updates)
        let route =
            Routing.findRoute(graph) a d DEFAULT_AMOUNT_MSAT 2 (Set.empty) (Set.empty) (Set.empty) DEFAULT_ROUTE_PARAMS (BlockHeight.FromBlocks 400000u)
            |> Result.deref
    ...
    testCase "find a route using channels with hltcMaximumMsat close to the payment amount" <| fun _ ->
        ...
        let graph = DirectedLNGraph.Create().AddEdges(updates)
        let route =
            Routing.findRoute graph f i DEFAULT_AMOUNT_MSAT 1 (Set.empty)(Set.empty)(Set.empty) DEFAULT_ROUTE_PARAMS (BlockHeight.FromBlocks 400000u)
            |> Result.deref
    ...
    testCase "no local channels" <| fun _ ->
        ...
        let g = DirectedLNGraph.Create().AddEdges(updates)
        let route = Routing.findRoute(g) a e DEFAULT_AMOUNT_MSAT 1 (Set.empty)(Set.empty)(Set.empty) DEFAULT_ROUTE_PARAMS (BlockHeight.FromBlocks 400000u)
        Expect.isError route ""
    ...
    testCase "route not found (source OR target node not connected)" <| fun _ ->
        ...
        Expect.isError(Routing.findRoute g a d DEFAULT_AMOUNT_MSAT 1 (Set.empty)(Set.empty)(Set.empty) DEFAULT_ROUTE_PARAMS (BlockHeight.FromBlocks 400000u)) ""
        Expect.isError(Routing.findRoute g b e DEFAULT_AMOUNT_MSAT 1 (Set.empty)(Set.empty)(Set.empty) DEFAULT_ROUTE_PARAMS (BlockHeight.FromBlocks 400000u)) ""
        
    testCase "route not found (amount too high OR too low)" <| fun _ ->
        let highAmount = DEFAULT_AMOUNT_MSAT * 10L
        let lowAmount = DEFAULT_AMOUNT_MSAT / 10L
        let updatesHi = [
            makeUpdateSimple(1UL, a, b)
        ...
        Expect.isError (Routing.findRoute gHigh a d highAmount 1 (Set.empty)(Set.empty)(Set.empty) DEFAULT_ROUTE_PARAMS (BlockHeight.FromBlocks 400000u)) ""
        Expect.isError (Routing.findRoute gLow a d lowAmount 1 (Set.empty)(Set.empty)(Set.empty) DEFAULT_ROUTE_PARAMS (BlockHeight.FromBlocks 400000u)) ""
    ...
    testCase "route to self" <| fun _ ->
        ...
        let g = DirectedLNGraph.Create().AddEdges(updates)
        let route =
            Routing.findRoute g a a DEFAULT_AMOUNT_MSAT 1 (Set.empty)(Set.empty)(Set.empty) DEFAULT_ROUTE_PARAMS (BlockHeight.FromBlocks 400000u)
        Expect.isError route ""
    ...
    testCase "route to immediate neighbor" <| fun _ ->
        ...
        let g = DirectedLNGraph.Create().AddEdges(updates)
        let route =
            Routing.findRoute(g) a b DEFAULT_AMOUNT_MSAT 1 (Set.empty)(Set.empty)(Set.empty) DEFAULT_ROUTE_PARAMS (BlockHeight.FromBlocks 400000u)
            |> Result.deref
    ...
    testCase "directed graph" <| fun _ ->
        ...
        Expect.sequenceEqual (hops2Ids(route1)) [1UL; 2UL; 3UL; 4UL] ""
        let route2 =
            Routing.findRoute g e e DEFAULT_AMOUNT_MSAT 1 (Set.empty)(Set.empty)(Set.empty) DEFAULT_ROUTE_PARAMS (BlockHeight.FromBlocks 400000u)
        Expect.isError route2 ""
    ...
    testCase "calculate route and return metadata" <| fun _ ->
        let uab =
            { UnsignedChannelUpdateMsg.ChainHash = Network.RegTest.GenesisHash
              ...
              MessageFlags = 0uy
              ChannelFlags = 0uy
              CLTVExpiryDelta = BlockHeightOffset16.FromBlocks 1us
    ...
    testCase "blacklist routes" <| fun _ ->
        ...
        let route1 = Routing.findRoute(g) a e DEFAULT_AMOUNT_MSAT 1 (Set.empty) (ignoredE) (Set.empty) DEFAULT_ROUTE_PARAMS (BlockHeight.FromBlocks 400000u)
    ...
    testCase "route to a destination that is not in the graph (with assisted routes)" <| fun _ ->
        ...
        let route1 =
            Routing.findRoute g a e DEFAULT_AMOUNT_MSAT 1 (extraGraphEdges) (Set.empty)(Set.empty) DEFAULT_ROUTE_PARAMS (BlockHeight.FromBlocks 400000u)
            |> Result.deref
        Expect.sequenceEqual(hops2Ids(route1)) [1UL; 2UL; 3UL; 4UL] ""
    ...
    testCase "Verify that extra hops takes precedence over known channels" <| fun _ ->
        let updates = [
            ...
            makeUpdate(2UL, b, c, LNMoney.MilliSatoshis 10L, 10u, None, None, None)
            makeUpdate(3UL, c, d, LNMoney.MilliSatoshis 10L, 10u, None, None, None)
            makeUpdate(4UL, d, e, LNMoney.MilliSatoshis 10L, 10u, None, None, None)
        ]
        ...
        let route1 =
            ...
            |> Result.deref
        Expect.sequenceEqual (hops2Ids(route1)) [1UL;2UL; 3UL; 4UL] ""
        Expect.equal ((route1 |> Seq.item 1).LastUpdateValue.FeeBaseMSat) (LNMoney.MilliSatoshis 10L) ""
        
        let (extraDesc, extraUpdate) = makeUpdate(2UL, b, c, LNMoney.MilliSatoshis 5L, 5u, None, None, None)
        let extraGraphEdges = Set.singleton({ GraphLabel.Desc = extraDesc; Update = extraUpdate })
        let route2 =
            Routing.findRoute g a e DEFAULT_AMOUNT_MSAT 1 extraGraphEdges (Set.empty) (Set.empty) DEFAULT_ROUTE_PARAMS (BlockHeight.FromBlocks 400000u)
            |> Result.deref
        Expect.sequenceEqual (hops2Ids(route2)) [1UL; 2UL; 3UL; 4UL] ""
        Expect.equal ((route2 |> Seq.item 1).LastUpdateValue.FeeBaseMSat) (LNMoney.MilliSatoshis 5L) ""
    ...
    testPropertyWithConfig fsCheckConfig "compute ignored channels" <| fun (f: NodeId, g:NodeId, h: NodeId, i: NodeId, j: NodeId) ->
        ...
        let updates = [
            makeUpdate(1UL, a, b, LNMoney.MilliSatoshis 10L, 10u, None, None, None)
            makeUpdate(2UL, b, c, LNMoney.MilliSatoshis 10L, 10u, None, None, None)
            makeUpdate(2UL, c, b, LNMoney.MilliSatoshis 10L, 10u, None, None, None)
            makeUpdate(3UL, c, d, LNMoney.MilliSatoshis 10L, 10u, None, None, None)
            makeUpdate(4UL, d, e, LNMoney.MilliSatoshis 10L, 10u, None, None, None)
            makeUpdate(5UL, f, g, LNMoney.MilliSatoshis 10L, 10u, None, None, None)
            makeUpdate(6UL, f, h, LNMoney.MilliSatoshis 10L, 10u, None, None, None)
            makeUpdate(7UL, h, i, LNMoney.MilliSatoshis 10L, 10u, None, None, None)
            makeUpdate(8UL, i, j, LNMoney.MilliSatoshis 10L, 10u, None, None, None)
        ]
        let publicChannels =
            channels
    ...
    testCase "limit routes to 20 hops" <| fun () ->
        ...
        let updates =
            ...
            |> Seq.mapi (fun i (na, nb) -> makeUpdate((uint64 i), na, nb, LNMoney.MilliSatoshis 5L, 0u, None, None, None) )
            
        let g = DirectedLNGraph.Create().AddEdges(updates)
        let r18 =
            (Routing.findRoute g (nodes.[0]) nodes.[18] DEFAULT_AMOUNT_MSAT 1 (Set.empty) (Set.empty) (Set.empty) DEFAULT_ROUTE_PARAMS (BlockHeight.FromBlocks 400000u))
            |> Result.deref
        Expect.sequenceEqual (hops2Ids(r18)) [ for i in 0..17 -> (uint64 i) ] ""
        let r19 =
            (Routing.findRoute g (nodes.[0]) nodes.[19] DEFAULT_AMOUNT_MSAT 1 (Set.empty) (Set.empty) (Set.empty) DEFAULT_ROUTE_PARAMS (BlockHeight.FromBlocks 400000u))
            |> Result.deref
        Expect.sequenceEqual (hops2Ids(r19)) [ for i in 0..18 -> (uint64 i) ] ""
        let r20 =
            (Routing.findRoute g (nodes.[0]) nodes.[20] DEFAULT_AMOUNT_MSAT 1 (Set.empty) (Set.empty) (Set.empty) DEFAULT_ROUTE_PARAMS (BlockHeight.FromBlocks 400000u))
            |> Result.deref
        Expect.sequenceEqual (hops2Ids(r20)) [ for i in 0..19 -> (uint64 i) ] ""
        let r21 =
            (Routing.findRoute g (nodes.[0]) nodes.[21] DEFAULT_AMOUNT_MSAT 1 (Set.empty) (Set.empty) (Set.empty) DEFAULT_ROUTE_PARAMS (BlockHeight.FromBlocks 400000u))
        Expect.isError(r21) ""
    ...
    testCase "ignore cheaper route when it has more than 20 hops" <| fun _ ->
        ...
        let route =
            Routing.findRoute g nodes.[0] nodes.[49] DEFAULT_AMOUNT_MSAT 1 (Set.empty)(Set.empty)(Set.empty) DEFAULT_ROUTE_PARAMS (BlockHeight.FromBlocks 400000u)
            |> Result.deref
    ...
    testCase "ignore cheaper route when it has more than the requested CLTV limit" <| fun _ ->
        let updates = [
            makeUpdate(1UL, a, b, LNMoney.One, 0u, Some(LNMoney.Zero), None, Some(BlockHeightOffset16.FromBlocks 50us))
            makeUpdate(2UL, b, c, LNMoney.One, 0u, Some(LNMoney.Zero), None, Some(BlockHeightOffset16.FromBlocks 50us))
            makeUpdate(3UL, c, d, LNMoney.One, 0u, Some(LNMoney.Zero), None, Some(BlockHeightOffset16.FromBlocks 50us))
            makeUpdate(4UL, a, e, LNMoney.One, 0u, Some(LNMoney.Zero), None, Some(BlockHeightOffset16.FromBlocks 9us))
            makeUpdate(5UL, e, f, LNMoney.MilliSatoshis 5L, 0u, Some(LNMoney.Zero), None, Some(BlockHeightOffset16.FromBlocks 9us))
            makeUpdate(6UL, f, d, LNMoney.MilliSatoshis 5L, 0u, Some(LNMoney.Zero), None, Some(BlockHeightOffset16.FromBlocks 9us))
        ]
        let g = DirectedLNGraph.Create().AddEdges(updates)
        let route =
            Routing.findRoute (g) a d DEFAULT_AMOUNT_MSAT 1 (Set.empty) (Set.empty) (Set.empty) {DEFAULT_ROUTE_PARAMS with RouteMaxCLTV = (BlockHeightOffset16.FromBlocks 28us)} (BlockHeight.FromBlocks 400000u)
            |> Result.deref
    ...
    testCase "ignore cheaper route when it grows longer than the requested size" <| fun _ ->
        let updates = [
            makeUpdate(1UL, a, b, LNMoney.One, 0u, Some(LNMoney.Zero), None, (Some (BlockHeightOffset16.FromBlocks 9us)))
            makeUpdate(2UL, b, c, LNMoney.One, 0u, Some(LNMoney.Zero), None, (Some (BlockHeightOffset16.FromBlocks 9us)))
            makeUpdate(3UL, c, d, LNMoney.One, 0u, Some(LNMoney.Zero), None, (Some (BlockHeightOffset16.FromBlocks 9us)))
            makeUpdate(4UL, d, e, LNMoney.One, 0u, Some(LNMoney.Zero), None, (Some (BlockHeightOffset16.FromBlocks 9us)))
            makeUpdate(5UL, e, f, LNMoney.One, 0u, Some(LNMoney.MilliSatoshis 5L), None, (Some (BlockHeightOffset16.FromBlocks 9us)))
            makeUpdate(6UL, b, f, LNMoney.One, 0u, Some(LNMoney.MilliSatoshis 5L), None, (Some (BlockHeightOffset16.FromBlocks 9us)))
        ]
        let g = DirectedLNGraph.Create().AddEdges(updates)
        let route =
            Routing.findRoute g a f DEFAULT_AMOUNT_MSAT 1 (Set.empty)(Set.empty)(Set.empty) { DEFAULT_ROUTE_PARAMS with RouteMaxLength = 3} (BlockHeight.FromBlocks 400000u)
            |> Result.deref
    ...
    testCase "ignore loops" <| fun _ ->
        let updates = [
            makeUpdate(1UL, a, b, LNMoney.MilliSatoshis 10L, 10u, None, None, None)
            makeUpdate(2UL, b, c, LNMoney.MilliSatoshis 10L, 10u, None, None, None)
            makeUpdate(3UL, c, a, LNMoney.MilliSatoshis 10L, 10u, None, None, None)
            makeUpdate(4UL, c, d, LNMoney.MilliSatoshis 10L, 10u, None, None, None)
            makeUpdate(5UL, d, e, LNMoney.MilliSatoshis 10L, 10u, None, None, None)
        ]
    ...
    testCase "ensure the route calculation terminates correctly when selecting 0-fees edges" <| fun _ ->
        ...
        let g = DirectedLNGraph.Create().AddEdges(updates)
        let route1 =
            Routing.findRoute g a d DEFAULT_AMOUNT_MSAT 1 (Set.empty)(Set.empty)(Set.empty) DEFAULT_ROUTE_PARAMS (BlockHeight.FromBlocks 400000u)
            |> Result.deref
    ...
    testCase "find the k shortest path (wikipedia example)" <| fun _ ->
        let updates = [
            makeUpdate(10UL, c, e, LNMoney.MilliSatoshis 2L, 0u, None,None,None)
            makeUpdate(20UL, c, d, LNMoney.MilliSatoshis 3L, 0u, None,None,None)
            makeUpdate(30UL, d, f, LNMoney.MilliSatoshis 4L, 5u, None,None,None) // D -> F has a higher cost to distinguish from the 2nd cheapest route
            makeUpdate(40UL, e, d, LNMoney.MilliSatoshis 1L, 0u, None,None,None)
            makeUpdate(50UL, e, f, LNMoney.MilliSatoshis 2L, 0u, None,None,None)
            makeUpdate(60UL, e, g, LNMoney.MilliSatoshis 3L, 0u, None,None,None)
            makeUpdate(70UL, f, g, LNMoney.MilliSatoshis 2L, 0u, None,None,None)
            
            makeUpdate(80UL, f, h, LNMoney.MilliSatoshis 1L, 0u, None,None,None)
            makeUpdate(90UL, g, h, LNMoney.MilliSatoshis 2L, 0u, None,None,None)
        ]
        
        let graph = DirectedLNGraph.Create().AddEdges(updates)
        let twoShortestPaths =
            Graph.yenKShortestPaths graph c h DEFAULT_AMOUNT_MSAT Set.empty Set.empty Set.empty 2 None BlockHeight.Zero (fun _ -> true)
            |> Seq.toList
    ...
    testCase "terminate looking for k-shortest path if there are no more alternative paths than k, must not consider routes going back on their steps" <| fun _ ->
        ...
        let updates = [
            makeUpdate(1UL, a, b, LNMoney.MilliSatoshis 1L, 0u, None, None, None)
            makeUpdate(1UL, b, a, LNMoney.MilliSatoshis 1L, 0u, None, None, None)
            makeUpdate(2UL, b, c, LNMoney.MilliSatoshis 1L, 0u, None, None, None)
            makeUpdate(2UL, c, b, LNMoney.MilliSatoshis 1L, 0u, None, None, None)
            makeUpdate(3UL, c, f, LNMoney.MilliSatoshis 1L, 0u, None, None, None)
            makeUpdate(3UL, f, c, LNMoney.MilliSatoshis 1L, 0u, None, None, None)
            makeUpdate(4UL, c, d, LNMoney.MilliSatoshis 1L, 0u, None, None, None)
            makeUpdate(4UL, d, c, LNMoney.MilliSatoshis 1L, 0u, None, None, None)
            makeUpdate(41UL, d, c, LNMoney.MilliSatoshis 1L, 0u, None, None, None) // there is more than one D -> C channel
            makeUpdate(5UL, d, e, LNMoney.MilliSatoshis 1L, 0u, None, None, None)
            makeUpdate(5UL, e, d, LNMoney.MilliSatoshis 1L, 0u, None, None, None)
            makeUpdate(6UL, e, f, LNMoney.MilliSatoshis 1L, 0u, None, None, None)
            makeUpdate(6UL, f, e, LNMoney.MilliSatoshis 1L, 0u, None, None, None)
        ]
        let graph = DirectedLNGraph.Create().AddEdges(updates)
        let foundPaths =
            Graph.yenKShortestPaths graph a f DEFAULT_AMOUNT_MSAT (Set.empty)(Set.empty)(Set.empty) 3 None BlockHeight.Zero (fun _ -> true)
            |> Seq.toList
    ...
    testCase "select a random route below the requested fee" <| fun _ ->
        let updates = [
            makeUpdate(1UL, a, b, LNMoney.MilliSatoshis 1L, 0u, None, None, None)
            makeUpdate(4UL, a, e, LNMoney.MilliSatoshis 1L, 0u, None, None, None)
            makeUpdate(2UL, b, c, LNMoney.MilliSatoshis 2L, 0u, None, None, None)
            makeUpdate(3UL, c, d, LNMoney.MilliSatoshis 3L, 0u, None, None, None)
            makeUpdate(5UL, e, f, LNMoney.MilliSatoshis 3L, 0u, None, None, None)
            makeUpdate(6UL, f, d, LNMoney.MilliSatoshis 3L, 0u, None, None, None)
            makeUpdate(7UL, e, c, LNMoney.MilliSatoshis 9L, 0u, None, None, None)
        ]
        let graph = DirectedLNGraph.Create().AddEdges(updates)
        let strictFeeParams = { DEFAULT_ROUTE_PARAMS with MaxFeeBase = LNMoney.MilliSatoshis 7L; MaxFeePCT = 0. }
        for _ in 0..10 do
            let r = Routing.findRoute graph a d DEFAULT_AMOUNT_MSAT 3 (Set.empty) (Set.empty) (Set.empty) strictFeeParams (BlockHeight.FromBlocks 400000u)
            Expect.isOk r ""
            let someRoute = r |> Result.deref
            let routeCost =
                (Graph.pathWeight (hops2Edges(someRoute)) DEFAULT_AMOUNT_MSAT false BlockHeight.Zero None).Cost - DEFAULT_AMOUNT_MSAT
    ...
    testCase "Use weight ratios to when computing the edge weight" <| fun _ ->
        ...
        let updates = [
            makeUpdate(1UL, a, b, LNMoney.MilliSatoshis 0L, 0u, Some(LNMoney.Zero), None, Some (BlockHeightOffset16.FromBlocks 13us))
            makeUpdate(4UL, a, e, LNMoney.MilliSatoshis 0L, 0u, Some(LNMoney.Zero), None, Some (BlockHeightOffset16.FromBlocks 12us))
            makeUpdate(2UL, b, c, LNMoney.MilliSatoshis 1L, 0u, Some(LNMoney.Zero), None, Some (BlockHeightOffset16.FromBlocks 500us))
            makeUpdate(3UL, c, d, LNMoney.MilliSatoshis 1L, 0u, Some(LNMoney.Zero), None, Some (BlockHeightOffset16.FromBlocks 500us))
            makeUpdate(5UL, e, f, LNMoney.MilliSatoshis 2L, 0u, Some(LNMoney.Zero), None, Some (BlockHeightOffset16.FromBlocks 9us))
            makeUpdate(6UL, f, d, LNMoney.MilliSatoshis 2L, 0u, Some(LNMoney.Zero), None, Some (BlockHeightOffset16.FromBlocks 9us))
            makeUpdate(7UL, e, c, LNMoney.MilliSatoshis 2L, 0u, Some(LNMoney.Zero), Some(largeCap), Some (BlockHeightOffset16.FromBlocks 12us))
        ]
        let graph = DirectedLNGraph.Create().AddEdges(updates)
        let r =
            Routing.findRoute graph a d DEFAULT_AMOUNT_MSAT 0 (Set.empty) (Set.empty) (Set.empty) DEFAULT_ROUTE_PARAMS (BlockHeight.FromBlocks 400000u)
            |> Result.deref
        ...
        let routeClTVOptimized =
            ...
            Routing.findRoute graph a d DEFAULT_AMOUNT_MSAT 0 (Set.empty) (Set.empty) (Set.empty)  p (BlockHeight.FromBlocks 400000u)
            |> Result.deref
    ...
    testCase "Prefer going through an older channel if fees and CLTV are the same" <| fun _ ->
        ...
        let mu(schid, one, two) = 
            makeUpdate2(schid, one, two, LNMoney.MilliSatoshis 1L, 0u, (Some LNMoney.Zero), None, (Some (BlockHeightOffset16.FromBlocks 144us)))
        let updates = [
        ...
        let routeScoreOptimized =
            ...
            Routing.findRoute graph a d (DEFAULT_AMOUNT_MSAT / 2L) 1 (Set.empty)(Set.empty)(Set.empty) rp (BlockHeight.FromBlocks currentBlockHeight)
            |> Result.deref
            |> hops2Nodes
    ...
    testCase "prefer a route with a smaller total CLTV if fees and scores are the same" <| fun _ ->
        let mu (schid, one, two, cltv) =
            makeUpdate(schid, one, two, LNMoney.One, 0u, (Some LNMoney.Zero), None, (Some (BlockHeightOffset16.FromBlocks cltv)))
        let updates = [
            mu(1UL, a, b, 12us)
            mu(4UL, a, e, 12us)
        ...
        let routeScoreOptimized =
            let wr = WeightRatios.TryCreate(0.33, 0.33, 0.33) |> Result.deref
            let rp = { DEFAULT_ROUTE_PARAMS with Ratios = Some (wr) }
            Routing.findRoute graph a d (DEFAULT_AMOUNT_MSAT / 2L) 1 (Set.empty)(Set.empty)(Set.empty) rp (BlockHeight.FromBlocks 400000u)
            |> Result.deref
            |> hops2Nodes
        Expect.sequenceEqual routeScoreOptimized [(a,b); (b,c); (c,d)] ""
    ...
    testCase "cost function is monotonic" <| fun _ ->
            ...
            let m =
                ...
                let unsignedChannelUpdate1 =
                    { UnsignedChannelUpdateMsg.ChainHash = uint256.Zero
                      ...
                      MessageFlags = 1uy
                      ChannelFlags = 0uy
                      CLTVExpiryDelta = BlockHeightOffset16.FromBlocks 14us
                ...
                let unsignedChannelUpdate2 =
                    { UnsignedChannelUpdateMsg.ChainHash = uint256.Zero
                      ...
                      MessageFlags = 1uy
                      ChannelFlags = 1uy
                      CLTVExpiryDelta = BlockHeightOffset16.FromBlocks 144us
            ...
            let m =
                ...
                let unsignedChannelUpdate1 =
                    { UnsignedChannelUpdateMsg.ChainHash = uint256.Zero
                      ...
                      ChannelFlags = 0uy
                      CLTVExpiryDelta = BlockHeightOffset16.FromBlocks 144us
                      HTLCMinimumMSat = LNMoney.Satoshis 1L
                      FeeBaseMSat = LNMoney.Satoshis(1L)
                      FeeProportionalMillionths = 100u
                ...
                let unsignedChannelUpdate2 =
                    { UnsignedChannelUpdateMsg.ChainHash = uint256.Zero
                      ...
                      CLTVExpiryDelta = BlockHeightOffset16.FromBlocks 144us
                      HTLCMinimumMSat = LNMoney.One
                      FeeBaseMSat = LNMoney.Satoshis 667L
        ...
        let routeParams =
            { RouteParams.Randomize = false
              ...
              MaxFeePCT = 0.03
              RouteMaxLength = 6
              RouteMaxCLTV = 1008us |> BlockHeightOffset16.FromBlocks
        ...
        let amount = LNMoney.MilliSatoshis 351000L
        let route =
            Routing.findRoute graph thisNode targetNode amount 1 (Set.empty)(Set.empty)(Set.empty) routeParams (BlockHeight.FromBlocks 567634u)
            |> Result.deref
            |> Seq.toList
